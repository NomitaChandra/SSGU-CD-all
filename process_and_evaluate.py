...
# os.chdir("/Users/kavithakamarthy/Downloads/SSGU-CD-all/data_processing")
# subprocess.run(["python", "process_cdr.py", "/Users/kavithakamarthy/Downloads/SSGU-CD-all/dataset/cdr"])
...
if args.task == 'cdr':
    ...
    args.model_name_or_path = '/Users/kavithakamarthy/Downloads/SSGU-CD-all/dataset/cdr/data/pretrained/scibert_scivocab_cased'
...
# output_csv_path = "test_features.csv"  # Change to your desired path
# test_features_df.to_csv(output_csv_path, index=False)
# print(f"Test features saved to {output_csv_path}")
...
model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
test_file = os.path.join(args.data_dir, args.test_file)
new_test_features = read(args, test_file, tokenizer, max_seq_length=args.max_seq_length)

for i in range(len(new_test_features)):
    new_test_features[i]['labels'] = test_features[i]['labels']  # Assuming new_test_features is a similar list

import pandas as pd
test_features_df = pd.DataFrame(new_test_features)
output_csv_path = "test_features_new.csv"  # Change to your desired path
test_features_df.to_csv(output_csv_path, index=False)
print(f"Test features saved to {output_csv_path}")
test_score, test_output, preds = evaluate(args, model, new_test_features, tag="test", generate=True)
print(test_score)
print(test_output)