            ...
            for i in range(self.rels):
                ...
                if self.args.m_tag == 'PN':
                    ...
                    risk1 = (((1. - self.priors_o[i]) / (1. - priors_u)) * self.square_loss(neg, -1., self.margin) -
                             ((priors_u - priors_u * self.priors_o[i]) / (1. - priors_u)) * self.square_loss(pos, -1.,
                                                                                                             self.margin))
                    ...
                    # 排除空关系
                    if neg.shape[0] == 0 and pos.shape[0] == 0:
                        print('pos and neg are null!')
                    elif neg.shape[0] == 0:
                        risk = risk2
                    elif pos.shape[0] == 0:
                        risk = risk1
                    if not risk > 10 and not risk <= 10:
                        print()
