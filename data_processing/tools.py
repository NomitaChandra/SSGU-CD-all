...
def generate_pairs_multi_entities(uents, type1, type2, true_rels):
    """
    Generate pairs (both positive & negative):
    Type1 - Type2 should have 1-1 association, e.g. [A, A] [B, C] --> (A,B), (A,C)
    Args:
    ...
    for c in combs:
        ...
        for tr in true_rels:
            if list(set(tr.arg1).intersection(set(c[0]))) and list(set(tr.arg2).intersection(set(c[1]))) or list(
                    set(tr.arg1).intersection(set(c[1]))) and list(set(tr.arg2).intersection(set(c[0]))):
                # AB existing relation
                for t1, t2 in zip(type1, type2):
                    if uents[c[0]][0].type == t1 and uents[c[1]][0].type == t2:
                        pairs[(c[0], c[1])] = \
        ...
        for term in ts:
            ...
            if newtext[term[0]:term[1]].replace(" ", "").replace("\n", "") != term[3].replace(" ", "").replace('\n',
                ...
                if newtext[term[0]:term[1]].replace(" ", "").replace("\n", "").lower() == \
                    ...
                    condition = True
                    tqdm.write(
                        'DOC_ID {}, Lowercase Issue: {} <-> {}'.format(term[4], newtext[term[0]:term[1]], term[3]))
                else:
                    condition = False
            else:
                condition = True
