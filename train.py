...
def train(args, model, train_features, dev_features, test_features):
    def finetune(features, optimizer, num_epoch, num_steps):
        ...
        for epoch in tqdm(train_iterator, desc="epoch"):
            ...
            test_model = amp.initialize(model, opt_level="O1", verbosity=0)
            test_model.load_state_dict(torch.load(args.save_path))
            test_score, test_output = evaluate(args, test_model, test_features, tag="test")
            print("test_score: ", test_score)
            print("test_output: ", test_output)
            if score_best < test_score:
...
def main():
    ...
    # novel = 'Novel' if args.novel else 'No'
    # sys.stdout = Logger(stream=sys.stdout, filename='./result/docred/' + timestamp + '.log')
